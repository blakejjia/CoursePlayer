...
class SongListPageBloc extends Bloc<SongListPageEvent, SongListPageState> {
  ...
  Future<void> _onLocateAlbum(event, emit) async {
    ...
    Uint8List? picture = await getIt<CoversRepository>()
        .getCoverUint8ListByPlaylist(event.album);
  ...
  Future<void> _onUpdateSongList(event, emit) async {
    if (state is SongListPageReady) {
      List<Song>? songList = await getIt<SongRepository>()
          .getSongsByAlbumId((state as SongListPageReady).album.id);
      Album? album = await getIt<AlbumRepository>()
          .getAlbumById((state as SongListPageReady).album.id);
      emit((state as SongListPageReady)
          .copyWith(buffer: songList, album: album));
    }
  }
}
