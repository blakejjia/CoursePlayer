...
class OnePlaylistBloc extends Bloc<OnePlaylistEvent, OnePlaylistState> {
  OnePlaylistBloc() : super(const OnePlaylistIdle(null, null, null)) {
    on<OnePlayListSelected>((event, emit) async{
      List<Song> songList= await getIt<LoadFromDb>().getSongsByPlaylist(event.playlist);
      //! sort below
      //! sort above
      Uint8List picture = await getIt<LoadFromDb>().getCoverUint8ListByPlaylist(event.playlist);
      emit(OnePlaylistIdle(event.playlist, songList, picture));
    });
    on<OnePlayListPlay>((event, emit){
      emit(OnePlaylistInAudio(state.currentPlaylist, state.songList, state.picture, event.index));
    });
  }
}
